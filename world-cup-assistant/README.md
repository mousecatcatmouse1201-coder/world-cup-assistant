...
本阶段保持原有 HTML、CSS、JavaScript 和 JSON 技术栈，主要完成以下页面体验优化：

1. 首页增加数据来源、数据更新时间和比赛总数状态栏。
2. 增加下一场比赛突出展示。
3. 优化“我的关注比赛”空状态和内容展示。
4. 增加一键清除筛选、只看关注球队和当前筛选条件反馈。
5. 全部赛程按北京时间日期分组展示。
6. 积分榜标题和自动计算说明更加清楚。
7. 增加加载、使用本地缓存和加载失败提示。
8. 优化手机端筛选布局、比赛卡片、按钮尺寸和积分榜横向滚动。
...
1. 数据状态栏和下一场比赛正常显示。
2. 球队、小组、状态和只看关注球队筛选正常。
3. 一键清除筛选可以恢复全部赛程。
4. 关注球队和收藏比赛正常，刷新后仍然保留。
5. 比赛按北京时间日期分组展示。
6. 小组积分榜正常计算。
7. 缩小浏览器宽度后页面正常，积分榜可以横向滚动。
8. 浏览器控制台没有错误。
